...
/**
 * Find all user-created variables which are in scope for a given block,
 *  optionally of specified type.
 * @param {!Blockly.Block|!Blockly.Workspace} block Block or workspace.
 * @param {string=} type Type of variables to return. If non-null, returns
 *    all variables of given type. If null, returns all variables of any type.
 * @return {!Array.<string>} Array of variable names.
 */
Blockly.Variables.scopeVariables = function(block, type) {
  var variableHash = Object.create(null);
  var workspace;
  if( block.getDescendants) {
    // block is a Block
    workspace = block.workspace;
    var parent = block;
    while( parent = parent.parentBlock_ ) {
      // Recurse upwards through parents and collect all variables
      if (parent.getVars) {
  ...
  } else if (block.getAllBlocks) {
    // block is a Workspace
    workspace = block;
  } else {
    throw 'Not Block or Workspace: ' + block;
  }
  // Add global variables
  if( workspace.globalVariables ) {
    console.log( "found global variables - scope" );
    for (var y = 0; y < workspace.globalVariables.length; y++) {
      if( workspace.globalVariables[y] instanceof String ) {
        /* Variable is untyped */
        if( !type ) {
